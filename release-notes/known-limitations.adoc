...
다른 Astra Control Center 인스턴스에서 클러스터를 관리하려면 먼저 link:../use/unmanage.html#unmanage-a-cluster["클러스터 관리를 취소합니다"] 현재 관리 중인 인스턴스에서 클러스터 관리를 해제해야 합니다. 클러스터를 관리에서 제거한 후에는 다음 명령을 실행하여 클러스터가 관리 해제되었는지 확인하십시오.
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["최신 Astra Trident 버전을 설치합니다"^].
...
연결을 설정할 때 link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["올바른 값을 입력하십시오"]을(를) 확인하십시오.
