...
.ONTAP 클러스터
* * Astra Control Provisioner *: Astra Control Provisioner 버전 23.10 이상 또는 A link:../get-started/requirements.html["Astra Trident를 지원했습니다"] ONTAP를 백엔드로 활용하는 소스 및 타겟 Kubernetes 클러스터 모두에 있어야 합니다.
...
+
.. 를 사용하여 StorageClass 객체를 생성합니다 `trident.netapp.io/replication: true` 매개 변수.
+
.예
[listing]
----
...
* TridentMirrorRelationship의 _spec.state_field를 로 업데이트합니다 `promoted`.
...
. 보조 Kubernetes 클러스터에서 TridentMirrorRelationship의 _spec.mirror_field를 로 업데이트합니다 `reestablished`.
