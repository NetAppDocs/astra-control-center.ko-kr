...
NOTE: 버킷을 추가할 때는 올바른 버킷 공급자를 선택하고 해당 공급자에 대한 올바른 자격 증명을 제공해야 합니다. 예를 들어, UI에서는 NetApp ONTAP S3를 유형으로 선택하고 StorageGRID 자격 증명을 입력할 수 있지만, 이렇게 하면 해당 버킷을 사용하는 향후 모든 앱 백업 및 복원 작업이 실패하게 됩니다. link:../release-notes/known-issues.html["릴리즈 노트"]을 참조하십시오.
