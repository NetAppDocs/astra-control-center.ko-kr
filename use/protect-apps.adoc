...
=== [Tech preview] 네임스페이스의 예약된 백업을 생성합니다

클러스터에서 일부 또는 모든 네임스페이스의 예약된 자동 백업을 생성할 수 있습니다. 이러한 워크플로우는 NetApp에서 Python 스크립트와 페어링된 CR 파일로 제공됩니다. 워크플로우를 구성하고 설치하면 설치 중에 선택한 일정에 따라 네임스페이스 백업이 자동으로 생성됩니다. 전체 클러스터 백업 워크플로우를 사용하여 클러스터의 모든 네임스페이스를 보호하거나 레이블 기반 백업 워크플로우를 사용하는 특정 네임스페이스만 보호할 수 있습니다.

백업 일정 범위에 속하는 새 네임스페이스 또는 앱이 클러스터에 추가되면 자동으로 보호됩니다.

.예: 전체 클러스터 백업
예를 들어, 전체 클러스터 백업 워크플로우를 구성하고 설치하면 네임스페이스의 모든 앱이 관리자의 추가 작업 없이 주기적으로 백업됩니다. 워크플로를 설치할 때 네임스페이스가 존재할 필요가 없습니다. 나중에 네임스페이스가 추가되면 네임스페이스가 보호됩니다.
