...
먼저 해 link:../get-started/setup_overview.html#add-cluster["Astra Control 관리에 클러스터를 추가합니다"], 클러스터(Astra Control 외부)에 앱을 설치한 다음 Astra Control의 애플리케이션 페이지로 이동하여 앱과 리소스 관리를 시작할 수 있습니다.
...
먼저 해 link:../get-started/setup_overview.html#add-cluster["클러스터가 추가되었습니다"] Astra Control은 클러스터에서 앱을 설치하거나 기존 앱을 관리할 수 있습니다. 단일 네임스페이스로 범위가 지정된 모든 앱을 관리할 수 있습니다.
...
WARNING: 앱을 정의할 때 보호 정책이 있는 여러 앱에 Kubernetes 리소스를 포함하지 않아야 합니다. Kubernetes 리소스의 보호 정책이 중복되어 데이터 충돌이 발생할 수 있습니다. <<Example: Separate Protection Policy for different releases,모범 사례에 대해 자세히 알아보십시오.>>
...
. 앱을 선택합니다.
. Actions * 열의 메뉴에서 * Unmanage * 를 선택합니다.
. 정보를 검토합니다.
