...
[role="lead"]
먼저 해 link:../get-started/add-cluster.html["Astra Control 관리에 클러스터를 추가합니다"], 클러스터(Astra Control 외부)에 앱을 설치한 다음 Astra Control의 애플리케이션 페이지로 이동하여 앱과 리소스를 정의할 수 있습니다.
...
먼저 해 link:../get-started/add-cluster.html["클러스터가 추가되었습니다"] Astra Control은 클러스터에서 앱을 설치하거나 기존 앱을 관리할 수 있습니다. 하나 이상의 네임스페이스로 범위가 지정된 모든 앱을 관리할 수 있습니다.
...
* (선택 사항) 모든 link:../use/manage-apps.html#application-management-requirements["지원되는 Kubernetes 리소스"]에 대한 Kubernetes 레이블.
+
...
* link:../use/manage-apps.html#tech-preview-define-an-application-using-a-kubernetes-custom-resource["(기술 미리보기) Kubernetes 맞춤형 리소스를 사용하여 애플리케이션을 정의합니다"]
