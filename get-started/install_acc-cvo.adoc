...
RedHat OpenShift 클러스터와 NetApp Cloud Volumes ONTAP 커넥터를 설치할 수 있도록 충분한 IAM 역할 및 권한이 있는지 확인합니다.
...
그런 다음 AWS를 구성하여 가상 프라이빗 클라우드를 생성하고, EC2 컴퓨팅 인스턴스를 설정하고, AWS S3 버킷을 생성하고, ECR(Elastic Container Register)을 생성하여 Astra Control Center 이미지를 호스팅하고, 이미지를 이 레지스트리로 푸시합니다.
...
NOTE: ECR을 생성하지 않는 경우, Astra Control Center는 AWS 백엔드가 있는 CVO가 포함된 클러스터에서 모니터링 데이터에 액세스할 수 없습니다. 이 문제는 Astra Control Center를 사용하여 검색 및 관리하려는 클러스터에 AWS ECR 액세스 권한이 없을 때 발생합니다.

. ACC 이미지를 정의된 레지스트리로 푸시합니다.



NOTE: AWS ECR(Elastic Container Registry) 토큰이 12시간 후에 만료되어 클러스터 간 클론 작업이 실패합니다. 이 문제는 AWS용으로 구성된 CVO(Cloud Volumes ONTAP)에서 스토리지 백엔드를 관리할 때 발생합니다. 이 문제를 해결하려면 ECR을 다시 인증하고 클론 작업이 성공적으로 재개되도록 새로운 암호를 생성하십시오.
...
. 이 CVO 배포에서 모든 영구 볼륨 및 볼륨을 기록해 둡니다.
...
그런 다음 가상 프라이빗 클라우드를 생성하고, 컴퓨팅 인스턴스를 설정하고, Azure Blob S3 버킷을 생성하고, Astra Control Center 이미지를 호스팅하기 위해 ACR(Azure Container Register)을 생성하고, 이 레지스트리에 이미지를 푸시하도록 Azure를 구성합니다.
...
. 백업을 저장할 Azure Blob S3 버킷을 하나 이상 생성합니다.
...
.. 오른쪽 위 모서리에서 Trident 버전을 확인합니다.
.. NetApp을 공급자 로 보여주는 CVO 클러스터 스토리지 클래스를 참조하십시오.
