...
. 커넥터가 실행 중인지 확인하고 해당 커넥터로 전환합니다.
+
image:acc-cvo-azure-connectors.png["BlueXP의 커넥터 전환"]
...
+
image:acc-cvo-azure-working-environment.png["BlueXP에서 작업 환경 생성"]

. OpenShift 클러스터를 가져옵니다. 클러스터가 방금 생성한 작업 환경에 연결됩니다.
+
.. NetApp 클러스터 세부 정보를 보려면 * K8s * > * 클러스터 목록 * > * 클러스터 세부 정보 * 를 선택합니다.
+
image:acc-cvo-azure-connected.png["BlueXP에서 가져온 클러스터"]
