...
+
** GKE 클러스터의 경우 Astra Control Service가 사용합니다 https://["Google Cloud용 NetApp Cloud Volumes Service"^] 또는 Google 영구 디스크를 영구 볼륨의 스토리지 백엔드로 사용합니다.
** AKS 클러스터의 경우 Astra Control Service가 사용합니다 https://["Azure NetApp Files"^] 또는 Azure 관리 디스크를 영구 볼륨의 스토리지 백엔드로 사용합니다.
** Amazon EKS 클러스터의 경우 Astra Control Service가 사용합니다 https://["Amazon Elastic Block Store를 클릭합니다"^] 또는 https://["NetApp ONTAP용 Amazon FSx"^] 영구 볼륨의 스토리지 백엔드로 사용됩니다.
...
** 에서는 새 관리자 역할을 사용하여 를 설치합니다 https://["아스트라 트리덴트"^] 를 클릭하여 하나 이상의 스토리지 클래스를 생성합니다.
** NetApp 클라우드 서비스 스토리지 오퍼링을 스토리지 백엔드로 사용하는 경우, Astra Control Service는 Astra Trident를 사용하여 앱에 영구 볼륨을 프로비저닝합니다. Amazon EBS 또는 Azure 관리 디스크를 스토리지 백엔드로 사용하는 경우 공급자별 CSI 드라이버를 설치해야 합니다. 설치 지침은 에 나와 있습니다 https://["Amazon Web Services를 설정합니다"^] 및 https://["Azure 관리 디스크를 사용하여 Microsoft Azure를 설정합니다"^].
...
Astra Control Center를 설치하고 사용하려면 반드시 충족해야 합니다 https://["요구 사항"^].

Astra Control Center는 다음과 같이 높은 수준에서 작동합니다.

* 현지 환경에 Astra Control Center를 설치합니다. 에 대해 자세히 알아보십시오 https://["Astra Control Center를 설치합니다"^].
...
에 대해 자세히 알아보십시오 https://["Astra Control Center를 설정합니다"^].
